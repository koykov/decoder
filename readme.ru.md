...
Декодеры во многом похожи на библиотеку [dyntpl](https://github.com/koykov/dyntpl/blob/master/readme.ru.md) только
...
* уникальное имя
* данные - как правило это какая-то структура, но может быть чем угодно
* тип-инспектор
...
максимально быстро и с нулевым или около нулевым количеством дополнительной памяти". Проблема получения данных была решена
...
Здесь важно понять что происходит неявно для пользователя. За каждой из переменных закреплён свой инспектор:
...
В итоге данные скопированы (с буферизацией если необходимо) из `rvalue` в `lvalue`.
...
указания источника данных, через символ `|`, указывается модификатор, как вызов функции:
```
...
Декодеры не способны распарсить сложные условия, содержащие более одной проверки, пример:
```
...
Декодеры поддерживает тернарный оператор для простейших случаев присваивания. Например такое условие:
```
if x.a == 123 {
...
Возможности декодеров могут быть расширены посредством включения в проект модулей расширения. Это обычные пакеты Go,
в которых вызываются функции регистрации модификаторов (`RegisterModFn`, `RegisterModFnNS`) или регистрация
условных helper-ов (`RegisterCondFn`, `RegisterCondFnNS`) или прочие функции регистрации из decoder API. В итоге
...
* [decoder_vector](https://github.com/koykov/decoder_vector) предоставляет возможность использовать функционал векторных парсеров в декодерах.
* [decoder_i18n](https://github.com/koykov/decoder_i18n) предоставляет возможность использования i18n функций для перевода в декодерах.
...
```
и в декодерах станут доступны все возможности векторных парсеров.

Не стесняйтесь разрабатывать собственные модули расширения. В этом случае рекомендуется регистрировать их в собственном
пространстве имён, как [здесь](https://github.com/koykov/decoder_vector/blob/master/init.go#L15).

### Заключение

Декодеры не слишком удобны в использовании из-за разделения процесса на этапы парсинга и декодирования и в случаях когда
не встаёт вопрос с производительностью. Он хорошо себя покажет только в условиях хайлоада с требованием динамичности.
Используйте его с умом и удачи в декодровании.
